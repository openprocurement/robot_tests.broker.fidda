...
Підготувати клієнт для користувача
    ...
    Run Keyword If    '${username}' != 'fidda_Viewer'    Login    ${username}
...
Створити об'єкт МП
    ...
    Run keyword    fidda.Змінити дані профілю органу приватизації    ${tender_data.data.assetCustodian}
    ...
    Run keyword    fidda.Оновити сторінку з об'єктом МП    ${username}    ${assetID}
    [Return]    ${assetID}
...
Оновити сторінку з об'єктом МП
    ...
    Run keyword    fidda.Оновити дані
    Sleep    1
...
Пошук лоту по ідентифікатору
    [Arguments]    ${username}    ${tender_uaid}
    Run keyword    fidda.Оновити сторінку з лотом    ${username}    ${tender_uaid}

Пошук об’єкта МП по ідентифікатору
    [Arguments]    ${username}    ${tender_uaid}
    Run keyword    fidda.Оновити сторінку з об'єктом МП    ${username}    ${tender_uaid}
...
Отримати інформацію із об'єкта МП
    ...
    ${return_value}=    Run Keyword    fidda.Отримати інформацію про ${field_name}
    [Return]    ${return_value}
...
Завантажити ілюстрацію в об'єкт МП
    ...
    Run keyword    fidda.Пошук об’єкта МП по ідентифікатору    ${username}    ${tender_uaid}
...
Завантажити ілюстрацію в лот
    [Arguments]    ${username}    ${tender_uaid}    ${filepath}
    Run keyword    fidda.Пошук лоту по ідентифікатору    ${username}    ${tender_uaid}
...
Завантажити документ в умови проведення аукціону
    ...
    Run keyword    fidda.Завантажити документ в лот з типом    ${username}    ${tender_uaid}    ${filepath}    ${documentType}

Внести зміни в об'єкт МП
    [Arguments]    ${username}    ${tender_uaid}    ${field_name}    ${field_value}
    Run keyword    fidda.Змінити ${field_name} об'єкта МП    ${username}    ${tender_uaid}    ${field_value}

Внести зміни в лот
    [Arguments]    ${username}    ${tender_uaid}    ${fieldname}    ${fieldvalue}
    Run keyword    fidda.Змінити ${fieldname} лоту    ${username}    ${tender_uaid}    ${field_value}

Внести зміни в актив лоту
    [Arguments]    ${username}    ${item_id}    ${tender_uaid}    ${fieldname}    ${fieldvalue}
    Run keyword    fidda.Змінити поле ${fieldname} активу лоту    ${field_value}

Внести зміни в умови проведення аукціону
    [Arguments]    ${username}    ${tender_uaid}    ${fieldname}    ${fieldvalue}    ${auction_index}
    Run keyword    fidda.Змінити поле ${fieldname} аукціону ${auction_index}    ${field_value}
...
Внести зміни в актив об'єкта МП
    ...
    Run keyword    fidda.Змінити ${field_name} об'єкта МП    ${username}    ${tender_uaid}    ${item_id}    ${field_value}
...
Додати умови проведення аукціону номер 0
  ...
  Log    ${auction}
  fidda.Оновити сторінку з лотом    ${username}  ${tender_uaid}
  Натиснути    id=auction-0-update-btn
...
Отримати інформацію з активу лоту
    [Arguments]    ${username}    ${tender_uaid}    ${item_id}    ${field_name}
    ${return_value}=    Run keyword     fidda.Отримати значення поля ${field_name} щодо активу лоту    ${username}    ${tender_uaid}    ${item_id}
    [Return]    ${return_value}

Отримати інформацію із лоту
    [Arguments]    ${username}    ${tender_uaid}    ${field_name}
    ${return_value}=    Run keyword    fidda.Отримати інформацію щодо лоту про ${field_name}
    [Return]    ${return_value}
...
Пошук тендера по ідентифікатору
    [Arguments]    ${username}    ${tender_uaid}
    Run keyword    fidda.Оновити сторінку з тендером    ${username}    ${tender_uaid}

Отримати інформацію із тендера
    [Arguments]    ${username}    ${tender_uaid}    ${field_name}
    Run keyword    fidda.Оновити сторінку з тендером    ${username}    ${tender_uaid}
    ${return_value}=    Run keyword    fidda.Отримати значення поля ${field_name} тендеру
    [Return]    ${return_value}
...
Отримати інформацію із запитання
    ...
    Run keyword    fidda.Пошук тендера по ідентифікатору    ${username}    ${tender_uaid}
...
Подати цінову пропозицію
    ...
    Run keyword    fidda.Змінити дані профілю учасника    ${bid.data.tenderers[0]}
    Run keyword    fidda.Пошук тендера по ідентифікатору    ${username}    ${tender_uaid}
...
Отримати інформацію із документа
    ...
    ${return_value}=    Run keyword    fidda.Отримати значення поля ${field} документу    ${doc_id}
    [Return]    ${return_value}
...
Отримати інформацію із пропозиції
    ...
    Run keyword    fidda.Пошук тендера по ідентифікатору    ${username}    ${tender_uaid}
    Натиснути    id=bid-create-btn
    ${return_value}=    Run keyword    fidda.Отримати інформацію з пропозиції шодо ${field}
    [Return]    ${return_value}

fidda.Отримати інформацію з пропозиції шодо value.amount
...
Активувати контракт
    ...
    Run keyword    fidda.Пошук тендера по ідентифікатору    ${username}    ${contract_uaid}
